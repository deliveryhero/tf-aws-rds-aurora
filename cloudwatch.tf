locals {
  ...
  cloudwatch_create_alarms = "${var.create_resources ? var.cloudwatch_create_alarms ? 1 : 0 : 0}"
}

resource "aws_cloudwatch_metric_alarm" "disk_queue_depth" {
  count               = "${local.cloudwatch_create_alarms}"
  alarm_name          = "${aws_rds_cluster.this.id}-alarm-rds-writer-DiskQueueDepth"
  ...
  period              = "60"
  statistic           = "Maximum"
  threshold           = "${lookup(var.cloudwatch_alarm_default_thresholds, "disk_queue_depth", local.cloudwatch_alarm_thresholds["disk_queue_depth"])}"
  alarm_description   = "RDS Maximum DiskQueueDepth Alarm for ${aws_rds_cluster.this.id} writer"
...
resource "aws_cloudwatch_metric_alarm" "database_connections_writer" {
  ...
  period              = "60"
  statistic           = "Sum"
  threshold           = "${lookup(var.cloudwatch_alarm_default_thresholds, "database_connections", local.cloudwatch_alarm_thresholds["database_connections"])}"
...
resource "aws_cloudwatch_metric_alarm" "cpu_utilization_writer" {
  ...
  period              = "60"
  statistic           = "Maximum"
  threshold           = "${lookup(var.cloudwatch_alarm_default_thresholds, "cpu_utilization", local.cloudwatch_alarm_thresholds["cpu_utilization"])}"
...
resource "aws_cloudwatch_metric_alarm" "aurora_replica_lag" {
  ...
  period              = "60"
  statistic           = "Maximum"
  threshold           = "${lookup(var.cloudwatch_alarm_default_thresholds, "aurora_replica_lag", local.cloudwatch_alarm_thresholds["aurora_replica_lag"])}"
